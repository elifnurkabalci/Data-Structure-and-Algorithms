...
public class GUIVisualization extends JFrame {
    ...
    public GUIVisualization(String plotType, List<Long> avgTimes, List<Integer> treeSize, String s) {
        ...
        /* Ensure the provided lists are not null and have the same size */
        if (treeSize != null && avgTimes != null && treeSize.size() > 1) {
            this.dataPointsX = treeSize;
            this.dataPointsY = avgTimes;
        } else {
            System.err.println("Invalid data points provided. Using sample data.");
            /* Use sample data if provided lists are invalid */
            for (int i = 1; i <= 10; i++) {
                dataPointsX.add(i * 1000); /* Add sample x-axis data points */
                dataPointsY.add((long) (Math.log(i * 1000) * 1000)); /*
                                                                      * Add sample y-axis data points reflecting O(logn)
                                                                      */
            }
        }
        setTitle("Performance Graph Visualization " + s); /* Set the title of the window */
        ...
        if (plotType.equals("line")) {
            for (int i = 0; i < dataPointsX.size() - 1; i++) {
                ...
                int y1 = height - padding - labelPadding
                        - (int) ((Math.log(dataPointsY.get(i) + 1) * 1.0) / Math.log(getMaxYValue() + 1)
                                * (height - padding * 2 - labelPadding));
                int x2 = (i + 1) * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding
                        + labelPadding;
                int y2 = height - padding - labelPadding
                        - (int) ((Math.log(dataPointsY.get(i + 1) + 1) * 1.0) / Math.log(getMaxYValue() + 1)
                                * (height - padding * 2 - labelPadding));
    ...
    public static void main(String[] args) {
         ...
         * GUIVisualization frame = new GUIVisualization(plotType); // Create a new
         * instance of GUIVisualization
